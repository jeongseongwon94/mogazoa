...
export default function ProductModal({
	...
	const handleNameInputBlur = () => {
		if (name === "") {
			...
			existingProductName.includes(name) &&
			(type === "add" || (type === "edit" && name !== productDetail.name))
		) {
	...
	const handleDropDownClick = (value: string) => {
		setName(value);
		if (
			existingProductName.includes(value) &&
			(type === "add" || (type === "edit" && name !== productDetail.name))
		) {
			setNameError("이미 등록된 상품입니다.");
		} else {
	...
	const onSubmit = async () => {
		...
		if (nextProductImage) {
			try {
				const imageUploadResult = await imageUploadMutation.mutateAsync(
					nextProductImage as File | null,
				);
		...
		<div className="w-[100%] px-[4rem] md:w-[59rem] lg:w-[62rem]">
			...
			<div className="relative flex flex-col gap-[3rem]">
				<div className="flex w-[100%] flex-row justify-between">
					<div className="mr-[2rem] flex w-[100%] flex-col gap-[3rem] md:m-0 md:w-[36rem]">
						<div className="relative">
							...
							/>
							{nameError && (
								<p className="absolute top-[8rem] text-[1.3rem] text-red">
									{nameError}
								</p>
							)}
						</div>
						<div>
							<AddCategoryDropdown
								...
								categoryId={type === "edit" ? categoryId : undefined}
							/>
							{categoryError && (
								<p className="absolute top-[17.5rem] text-[1.3rem] text-red">
									{categoryError}
								</p>
							)}
						</div>
				...
				>
					<textarea
						value={description}
						className="w-full resize-none overflow-y-scroll border-none bg-[#252530] text-[1.4rem] text-white scrollbar-hide placeholder:text-[1.4rem] placeholder:text-gray-200 focus:border-main_blue focus:outline-none lg:text-[1.6rem] lg:placeholder:text-[1.6rem]"
						rows={3}
				...
				{textareaError && (
					<p className="absolute top-[35rem] text-[1.3rem] text-red">
						{textareaError}
					</p>
				)}
				{imageError && (
					<p className="absolute top-[37rem] text-[1.3rem] text-red">
						{imageError}
					</p>
				)}
				<BasicButton
